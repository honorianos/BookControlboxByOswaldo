...
class Items1Cell1: UICollectionViewCell {
	...
	//-------------------------------------------------------------------------------------------------------------------------------------------
	func bindData(index: Int, data: Libros) {

		imageProduct.sample("Ecommerce", "Shoes", index + 4)
        labelTitle.text = data.nombre
        labelBrandName.text = data.autor
        labelPrice.text = data.categoria
        imageProduct.kf.setImage(with: URL(string: data.foto))
